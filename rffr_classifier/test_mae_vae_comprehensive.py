#!/usr/bin/env python3
"""
Comprehensive test script for MAE-VAE 3-branch (wavelet) model.
...
def parse_args_early():
    parser = argparse.ArgumentParser(description="Comprehensive test for MAE-VAE model")
    parser.add_argument("--checkpoint", type=str, required=True)
    parser.add_argument("--batch_size", type=int, default=32)
    parser.add_argument("--gpu", type=str, default="0")
    parser.add_argument(
        "--output", type=str, default="test_mae_vae_3branch_comprehensive.json"
    )
...
# CRITICAL: Configure for MAE-VAE BEFORE importing anything else from RFFR
...
# Set MAE-VAE specific settings on the config singleton
config.generative_model_type = "mae_vae"
config.mae_path = "/seidenas/users/nmarini/generative_checkpoint/mae_vae/FFHQ_mae_vae_STAGE1_best/best_loss_0.01325_800.pth.tar"
config.use_wavelets = True
config.wavelet_residual_branch = True
config.classifier_uses_wavelets = True
...
def main():
    ...
    results["datasets"]["CelebDF"] = {"auc": round(auc, 4), "samples": n_samples}
    ...
    results["datasets"]["DFD"] = {"auc": round(auc, 4), "samples": n_samples}
    ...
    results["datasets"]["DFDC"] = {"auc": round(auc, 4), "samples": n_samples}
    ...
    cross_dataset = {
        k: v for k, v in results["datasets"].items() if k in ["CelebDF", "DFD", "DFDC"]
    }
    ffhq_results = {
        k: v for k, v in results["datasets"].items() if k.startswith("FFHQ_")
    }
    ...
    ffhq_in_dist = {
        k: v
        for k, v in ffhq_results.items()
        if any(x in k for x in ["StyleGAN1", "StyleGAN2", "StyleGAN3"])
        and "XL" not in k
    }
    ffhq_ood = {
        k: v for k, v in ffhq_results.items() if any(x in k for x in ["XL", "SD_"])
    }
