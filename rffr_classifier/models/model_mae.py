...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(
        ...
        self.magic = torch.Tensor(
            [0, 1, 2, 14, 15, 16, 28, 29, 30]
        )  # Offsets against up left corner

        # Just the index of all blocks.
        self.batch = (
    ...
    def random_masking(self, x, block_id=None):
        """
        ...
        # block_id = None if Performing random blocking
        # block_id = 0~15 if uniformly block one position.
        ...
        if block_id is None:
            ...
            # block_id is a N-dimension vector now.
        ...
        # Totally 196 blocks, subtracted by 9 masked blocks.
        len_keep = 187
    ...
    def forward_encoder(self, x, block_id=None):
        # embed patches
        ...
        x, mask, ids_restore = self.random_masking(x, block_id)
    ...
    def forward(self, imgs, block_id=None):

        latent, mask, ids_restore = self.forward_encoder(imgs, block_id)
    ...
    def patch_by_patch_DIFF(
        self, data, block_id=None, show=False, test=False, return_reconstructed=False
    ):
        ...
        for i in range(num_block):
            ...
            h = 3 * ((block_id[:, i]) % 4) + 1
            _, pred, _ = self(data.float(), block_id=block_id[:, i])
