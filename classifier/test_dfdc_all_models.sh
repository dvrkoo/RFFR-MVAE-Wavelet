...
# Test all 5 RFFR models on DFDC dataset

set -e
cd "$(dirname "$0")"

SAMPLES=860  # DFDC has only 860 real samples, so we balance with 860 fake samples
...
# Test all 5 models sequentially on DFDC
...
echo ""
echo "Results:"
ls -lh ./test_results/*DFDC*.json 2>/dev/null | tail -5
