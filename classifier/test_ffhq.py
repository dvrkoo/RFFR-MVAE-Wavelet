...
def create_ffhq_dataset(ffhq_root, generator, split='test_set'):
    ...
    Args:
        ...
        generator: Generator name (e.g., 'stylegan2-psi-0.5') or 'all' for all generators
        split: 'test_set', 'val_set', or 'train_set'
    
    Returns:
        List of dicts with 'path' and 'label' keys
    """
    # Define all generators for FFHQ
    ALL_GENERATORS = [
        'stylegan1-psi-0.5',
        'stylegan2-psi-0.5', 
        'stylegan3-psi-0.5',
    ...
    # Get real images once (same for all generators)
    ...
    # Handle 'all' generator - load from all generators
    if generator.lower() == 'all':
        print(f"  Loading from ALL generators:")
        all_fake_images = []
        
        for gen in ALL_GENERATORS:
            fake_dir = Path(ffhq_root) / 'generated' / gen / 'images1024x1024' / split
            
            if not fake_dir.exists():
                print(f"    Warning: Fake images directory not found: {fake_dir} (skipping)")
                continue
            
            fake_images = sorted(list(fake_dir.glob('*.png')) + list(fake_dir.glob('*.jpg')))
            all_fake_images.extend(fake_images)
            print(f"    {gen}: {len(fake_images)} images")
        
        # Add all fake images (label=1)
        for img_path in all_fake_images:
            data_dict.append({
                'path': str(img_path),
                'label': 1
            })
        ...
        print(f"  Total fake images (all generators): {len(all_fake_images)}")
...
def test_generator(model, args, generator):
    ...
    try:
        ...
        dataset = Deepfake_Dataset(data_dict, train=False, cache_in_memory=False)
        ...
        with torch.no_grad():
            for normed, unnormed, labels in tqdm(dataloader, desc=f"Testing {generator}"):
                ...
                # Forward pass - RFFR returns (features, output, num_blocks)
                # output has shape (batch_size, 2) with logits for [real, fake]
                outputs = model(normed, unnormed, test=True)
                
                # Get logits (second output)
                if isinstance(outputs, tuple) and len(outputs) >= 2:
                    logits = outputs[1]  # Shape: (batch_size, 2)
                else:
                    logits = outputs
                
                # Apply softmax to get probabilities and take fake probability (class 1)
                probs = F.softmax(logits, dim=1)[:, 1]  # Shape: (batch_size,)
